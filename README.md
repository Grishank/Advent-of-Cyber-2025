<p align="center">
  <img src="./assets/images/aoc-banner.png" width="900"/>
</p>

# 🎄 Advent of Cyber 2025 — TryHackMe (My Complete Write-Ups)

Welcome to my **Advent of Cyber 2025** repository!  
This repo contains all write-ups, screenshots, and notes for **TryHackMe’s biggest annual cybersecurity challenge**.

TryHackMe releases **one challenge per day from December 1st to December 25th**, covering:

- SOC & Blue Team Analysis  
- Pentesting Basics  
- OSINT  
- Digital Forensics  
- Threat Detection  
- Web Security  
- Malware & Reverse Engineering  
- Logging & SIEM Basics  

By completing all 25 days, participants earn a **completion certificate**, a badge, and a strong portfolio project.

🏅 This repository includes the official Advent of Cyber 2025 completion certificate and full daily write-ups.
---

# 🏆 Advent of Cyber 2025 Certificate

<p align="center">
  <img src="./assets/images/aoc-2025-certificate.jpg" width="900"/>
</p>

✔ Completed all 25 days of **TryHackMe – Advent of Cyber 2025**  
✔ Hands-on SOC, Blue Team, Forensics, Web & Cloud challenges  
✔ Public write-ups + evidence for every day  

🔗 **TryHackMe profile (Verification): https://tryhackme.com/p/grishdubey01

---

# 🟣 Prep Track (Completed)

Before Day 1, TryHackMe provides a **Prep Track** — a warm-up room with mini-tasks introducing Linux/Windows fundamentals, investigation basics, WiFi concepts, OSINT clues, and platform usage.

✔ Completed: Advent of Cyber 2025 — Prep Track  
📄 Write-up: [Prep Track](./Prep-Track/prep-track.md)

---

# 📁 Folder Structure

```
Prep-Track/
Day-1/
Day-2/
Day-3/
Day-4/
Day-5/
Day-6/
Day-7/
Day-8/
Day-9/
...
Day-24/
```

Inside each day folder:

- **dayX.md** → Write-up  
- **images/** → Screenshots  

Example:
```
Day-1/
    day1.md
    images/
```

---

# 📅 Progress Tracker

### ✔ Prep Track — Completed  
📄 [Prep Track](./Prep-Track/prep-track.md)

### ✔ Day 1 — Completed  
📄 [Day 1 Write-up](./Day-1/day1.md)

### ✔ Day 2 — Completed  
📄 [Day 2 Write-up](./Day-2/day2.md)

### ✔ Day 3 — Completed  
📄 [Day 3 Write-up](./Day-3/day3.md)

### ✔ Day 4 — Completed  
📄 [Day 4 Write-up](./Day-4/day4.md)

### ✔ Day 5 — Completed  
📄 [Day 5 Write-up](./Day-5/day5.md)

### ✔ Day 6 — Completed  
📄 [Day 6 Write-up](./Day-6/day6.md)

### ✔ Day 7 — Completed  
📄 [Day 7 Write-up](./Day-7/day7.md)

### ✔ Day 8 — Completed  
📄 [Day 8 Write-up](./Day-8/day8.md)

### ✔ Day 9 — Completed  
📄 [Day 9 Write-up](./Day-9/day9.md)

### ✔ Day 10 — Completed  
📄 [Day 10 Write-up](./Day-10/day10.md)

### ✔ Day 11 — Completed  
📄 [Day 11 Write-up](./Day-11/day11.md)

### ❄ Day 12 — Completed
📄 [Day 12 Write-up](./Day-12/day12.md)

### ❄ Day 13 — Completed
📄 [Day 13 Write-up](./Day-13/day13.md)

### ❄ Day 14 — Completed
📄 [Day 14 Write-up](./Day-14/day14.md)

### ❄ Day 15 — Completed
📄 [Day 15 Write-up](./Day-15/day15.md)

### ❄ Day 16 — Completed
📄 [Day 16 Write-up](./Day-16/day16.md)

### ❄ Day 17 — Completed
📄 [Day 17 Write-up](./Day-17/day17.md)

### ❄ Day 18 — Completed
📄 [Day 18 Write-up](./Day-18/day18.md)

### ❄ Day 19 — Completed
📄 [Day 19 Write-up](./Day-19/day19.md)

### ❄ Day 20 — Completed
📄 [Day 20 Write-up](./Day-20/day20.md)

### ❄ Day 21 — Completed
📄 [Day 21 Write-up](./Day-21/day21.md)

### ❄ Day 22 — Completed
📄 [Day 22 Write-up](./Day-22/day22.md)

### ❄ Day 23 — Completed
📄 [Day 23 Write-up](./Day-23/day23.md)

### ❄ Day 24 — Completed
📄 [Day 24 Write-up](./Day-24/day24.md)

---

# 🎯 Why Advent of Cyber?

Advent of Cyber is one of the most practical, hands-on cybersecurity events for Beginners → Intermediate learners. Completing it helps build:

- Consistent daily learning habits  
- Real SOC investigation workflows  
- Practical Linux/Windows skills  
- OSINT and forensics thinking  
- Pentesting and threat analysis  
- Portfolio-ready technical documentation  

This repository serves as a **public record of my problem-solving journey**.

---

## 🔥 Part of My 60-Day SOC + AI Journey

This challenge is integrated into my broader **SOC + AI learning roadmap**, helping me level up:

- Detection & response thinking  
- Tool-based investigations  
- Documentation discipline  
- Real-world security workflows  

---
